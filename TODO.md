...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| F1 | W1 | Fundação | Estrutura de diretórios + `.claude-plugin/plugin.json` (name=bigtech, Apache-2.0) + `marketplace.json` (1 plugin, `source: "./"`). **One-way-door.** | Alta | — | Baixa | ⏳ Pendente | — |
| R1 | W2 | Release | `LICENSE` Apache-2.0 + `NOTICE` (pull-early: desbloqueia AUD-LICENSE). | Alta | F1 | Baixa | ⏳ Pendente | — |
...
| D1a | W2 | Docs | Higienizar `TOOLING.md` (40 wikilinks — o doc mais pesado). | Alta | F1 | Alta | ⏳ Pendente | — |
...
| H1 | W2 | Hooks | Portar hooks TDD (`tdd_guard`+`tdd_common`+`tdd_runner`+`tests/`); paths → `${CLAUDE_PLUGIN_ROOT}`. | Média | F1 | Alta | ⏳ Pendente | — |
| H2 | W2 | Hooks | Portar `bigtech_porte_reminder` + `bigtech_reinforce` (paths/marcador `.bigtech-porte`). | Média | F1 | Média | ⏳ Pendente | — |
| H3 | W2 | Hooks | Criar `bigtech_session_init.py` (SessionStart: docs-bootstrap + aviso caveman + sugestão deps). **Caminho crítico (§4.3).** | Alta | F1, D3 | Alta | ⏳ Pendente | — |
| D4 | W3 | Docs | Reescrever ORG §0 — transferência do título de líder supremo/CEO ao usuário que instala. | Média | D1b | Média | ⏳ Pendente | — |
| H4 | W3 | Hooks | `hooks/hooks.json` — registro central de todos os hooks (eventos + `${CLAUDE_PLUGIN_ROOT}`). | Média | H1, H2, H3 | Baixa | ⏳ Pendente | — |
