...
3. **Em modo redesign:** localize as telas/codigo atuais (`Glob`/`Grep`/`Read`) e, quando util, capture a tela atual para o diagnostico antes/depois - caminho primario: MCP `chrome-devtools` (`navigate_page` + `take_screenshot`, funciona nos 3 SO); fallback por SO: Linux (`grim`/`spectacle`/`maim`/`scrot`), macOS (`screencapture`), Windows (chrome-devtools cobre; Snipping Tool nativo como ultimo recurso).
